		...
		try{
			...
			for each (System::Windows::Forms::ListViewItem^ item in selectedItem)
			{
				...
				if (flag == 0){
					if (item->Checked)
					{
						Directory::Move(textBox1->Text + name, textBox1->Text + newName);   //вызов функции удаления папки
					}
					else
					{
						File::Move(textBox1->Text + name, textBox1->Text + newName);      //вызов функции удаления файла
					}
				}
				else {
					if (item->Checked)
